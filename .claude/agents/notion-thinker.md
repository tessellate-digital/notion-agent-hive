...
4. **Ask the user** via `AskUserQuestion` if the investigation reveals ambiguity that only the user can resolve.
...
5. **Use built-in Explore agent (and any available MCP-backed code search tools) liberally** to gather codebase context. The more concrete references in your reports, the better.
