...
- **Thinker** (via {{AGENT_TOOL}}): Deep research, planning, and task decomposition
...
Spawn a `notion-thinker` subagent via the {{AGENT_TOOL}} with this prefix:
...
2. **Dispatch an executor subagent** via the {{AGENT_TOOL}}. Pass:
...
1. **Dispatch a reviewer subagent** via the {{AGENT_TOOL}}. Pass:
...
When multiple tasks are independent (no dependency relationship), you MAY dispatch multiple executor subagents in parallel via the {{AGENT_TOOL}}. Update each task status independently as subagents complete.
