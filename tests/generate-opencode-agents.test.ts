...
describe('OpenCode Agent Generator', () => {
  beforeAll(() => {
    // Run the generator using the same CLI command a maintainer would run
    execSync('npm run generate:opencode', { cwd: root, stdio: 'pipe' });
  });
  ...
  describe('INSTALL.md', () => {
    ...
    it('should contain platform-specific destination path (.opencode/)', () => {
    ...
    it('should contain copy-oriented install language', () => {
      ...
      expect(content).toMatch(/cp\s+\.opencode\/agents\/.*\.md/);
    ...
    it('should mention OpenCode platform', () => {
      const content = readFileSync(installPath, 'utf-8');
