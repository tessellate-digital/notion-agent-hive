...
describe("ForegroundFallbackManager", () => {
  it("does nothing when disabled", async () => {
    ...
    const result = await manager.handleEvent({ type: "error", properties: { error: { code: "rate_limit_exceeded" }, agentName: "notion-thinker" } })
  ...
  it("does nothing for non-rate-limit errors", async () => {
    ...
    const result = await manager.handleEvent({ type: "error", properties: { error: { code: "invalid_request" }, agentName: "notion-thinker" } })
    expect(result).toBeUndefined()
  })
  ...
  it("returns next model in chain on rate limit", async () => {
    ...
    const manager = new ForegroundFallbackManager(chains, true)
    const event = { type: "error", properties: { error: { code: "rate_limit_exceeded" }, agentName: "notion-thinker" } }
