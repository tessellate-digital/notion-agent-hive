...
> **Version 0.3.0** — New: Thinker-Scoper for multi-vertical feature triage, Stacked PR Architect for `gh stack` workflows, Released status for shipped tasks, and per-task Repo tagging.
...
| **Thinker — Scoper** | Fast scope triage for multi-vertical features. Determines whether a feature spans multiple repos/services and whether to dispatch one planner or several. Never produces a full plan. |
...
| **Executor** | Implements code for the specific ticket assigned by the coordinator. Runs a mandatory reuse scan before writing anything new — avoids duplicating existing utilities and types. Tags each task with its Repo. Reports back with a verdict. |
...
| **Reviewer — Final** | Feature-level coherence review after all tasks pass individual review. Looks for integration gaps (A works, B works, A→B is broken), cross-task style drift, untested module seams, and cross-task duplication. Skips Released tasks. |
...
| **Stacked PR Architect** | Groups changes into ordered `gh stack` layers for independently reviewable stacked PRs. Produces a plan, waits for approval, then builds the local branch stack. Never pushes or submits PRs. GitHub-only. |
...
A few hard rules are baked into every agent: no git write commands without explicit user approval, no moving tasks to Done or Released (human only), no skipping review gates. The final reviewer exists specifically to catch what per-task reviews structurally cannot — integration gaps, cross-task style drift, and duplicated code that looked fine in isolation (and skips Released tasks that are already shipped). The PR reviewer can fetch GitHub review comments, investigate the code they reference, and classify them based on evidence rather than reviewer assertions.
...
| `thinker` | Planning, scoping, investigation, refinement | Strong reasoning model (e.g., gpt-5.4, opus-4.6) |
...
| `stackedPrArchitect` | Stacked PR workflows via `gh stack` | Strong reasoning model (e.g., opus-4.6) |
